...
if (1) {
    filelist.forEach(name => {
        const bytes = fs.readFileSync(`sources/3_automne/${name}`);
        ...
        fs.writeFileSync(`disasm/3_automne/${name}.json`, disasm);
    });
} else {
    const igaData = igarchive.igaCreate(
        filelist.map(name => {
            const text = fs.readFileSync(`disasm/3_automne/${name}.json`, { encoding: 'utf-8' });
